...
//            Utils.setCookie(response, "pplsEntrance", "true", null/*"/PplsService/"*/, null/*MessageUtil.getProperty("server.domain")*/, false, 3);
            ...
            superUrlString = MessageUtil.getProperty("entranceLink");
    ...
    public UserAccount prepareEntrance(HttpServletRequest request, HttpServletResponse response, ClientDetails clientDetails){
        return prepareEntrance(request, response, clientDetails, true);
    }

    public UserAccount prepareEntrance(HttpServletRequest request, HttpServletResponse response, ClientDetails clientDetails, boolean storeUserAccount){
        UserAccount userAccount = generateUserAccount(request, clientDetails);
    ...
    public  UserAccount generateUserAccount(HttpServletRequest request, ClientDetails clientDetail){
        UserAccount userAccount = generateUserAccount(clientDetail);
        if(request.getMethod().equals("GET"))
            userAccount.setAutorizeUrl(Utils.getQueryUrl(request));
        else
            userAccount.setAutorizeUrl(Utils.getQueryString(request));

        if(Boolean.valueOf(request.getParameter(AGENT_CHECK)) && Roles.convertAuthorityToRoles(clientDetail.getAuthorities()).contains(Roles.ROLE_APP))
            userAccount.setAuthType(AuthType.agentOnly);
        else
            userAccount.setAuthType(AuthType.full);
        return userAccount;
    }


    public static UserAccount generateUserAccount(ClientDetails clientDetail){
        UserAccount userAccount = new UserAccount();
    ...
    private static int computeMaxPossibleSessionExpire(ClientDetails clientDetail){
        int maxSessionExpire = SecurityConfig.accessTokenValiditySeconds+SecurityConfig.refreshTokenValiditySeconds;
        int accessValiditySeconds = clientDetail.getAccessTokenValiditySeconds()==null ? 0 : clientDetail.getAccessTokenValiditySeconds();
        int refreshValiditySeconds = clientDetail.getRefreshTokenValiditySeconds()==null ? 0 : clientDetail.getRefreshTokenValiditySeconds();
